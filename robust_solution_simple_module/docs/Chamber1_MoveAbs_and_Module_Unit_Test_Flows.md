# Chamber1 MoveAbs Service 與 Module 單元測試驗證流程

## 1. 文件目的

本文件描述下列兩個 TcUnit Test Suite 目前各 Test Method 的驗證目的、測試資料、逐 scan 流程與主要斷言：

- `FB_Chamber1SpinAxisMoveAbsServiceTests`
- `FB_Chamber1_ModuleTests`

對應受測物件：

- `Untitled1/POUs/10_Module/11_Chamber1/Services/FB_Chamber1SpinAxisMoveAbsServices.TcPOU`
- `Untitled1/POUs/10_Module/11_Chamber1/FB_Chamber1_Module.TcPOU`
...
- Module 測試不可另外呼叫 `FakeAxis.M_CyclicUpdate()`；`FB_Chamber1_Module.H_UpdateBaseUnit()` 已在每個有效 Module scan 更新 Spin 與 LiftPin BaseUnit 各一次。
...
- `FB_Chamber1_Module` 與 `FB_FakeAxis_BaseUnit` 都宣告在 `FB_Chamber1_ModuleTests` 的 suite-level `VAR`。
...
- 本批 Module 測試不使用 `FB_TestableChamber1Module`。

---

## 3. FB_Chamber1SpinAxisMoveAbsServiceTests
...
- Spin Axis owner 為 `E_Chamber1ServiceId.SpinAxisMoveAbs`。
